...
export class Files extends BaseResource {
    ...
    async createLargeUpload(request: CreateLargeFileUploadRequest): Promise<CreateLargeFileUploadResponse> {
        const response = await this.client.post<CreateLargeFileUploadResponse>("/file/large-file/create", request);
        return response.data;
    }
    ...
    async getLargeUploadProgress(uploadId: string): Promise<LargeFileUploadProgressResponse> {
        const response = await this.client.get<LargeFileUploadProgressResponse>("/file/large-file/progress", {
    ...
    async completeLargeUpload(uploadId: string): Promise<CompleteLargeFileUploadResponse> {
        const response = await this.client.post<CompleteLargeFileUploadResponse>("/file/large-file/complete", {
            upload_id: uploadId,
        });
        return response.data;
    }
